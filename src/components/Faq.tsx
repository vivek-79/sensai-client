...
const Faq = () => {
  return (
    <section className="w-full flex-col items-center justify-center">
      <h2 className="pt-18 text-white text-center text-3xl md:text-4xl font-bold">Frequently Asked Questions</h2>
      <p className="text-white/40 text-center mt-4">Find answer to common questions about our platform</p>
      <div className="mx-auto w-full max-w-[500px] py-8 px-4 flex-col items-center justify-center  text-white">
        {faqs.map((item, indx) => (
          <details className=" mb-4 border-b-[1px] hover:bg-white/10 rounded-md p-2 transition-colors duration-300 border-white/30" key={indx}>
            <summary className="mb-4 font-medium">{item.question}</summary>
            <p className="mb-4 text-sm text-white/70">{item.answer}</p>
          </details>
        ))}
      </div>
    </section>
